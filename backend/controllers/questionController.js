...
const generateAIAnswer = async (title, category) => {
  try {
    const model = genAI.getGenerativeModel({ model: 'gemini-1.5-flash' });
    const prompt = `
      You are an expert Indian agricultural advisor helping farmers.
      A farmer asked: "${title}"
      Category: ${category}
      
      Give a helpful, specific, practical answer in simple English.
      Include:
      - Direct answer to the question
      - Specific product names or quantities if relevant
      - 2-3 actionable tips
      - Keep it under 150 words
      - End with: "⚠️ This is an AI-generated answer. Please wait for expert verification."
    `;
    const result = await model.generateContent(prompt);
    return result.response.text();
  } catch (error) {
    // Fallback to rule-based if Gemini fails
    return fallbackAnswer(title, category);
  }
};

const fallbackAnswer = (title, category) => {
  const q = title.toLowerCase();

  if (q.includes('yellow') && (q.includes('spot') || q.includes('leaf'))) {
    return `🤖 AI Answer: Yellow spots usually indicate Yellow Rust or Nutrient Deficiency.\n\n• Apply Propiconazole 25EC @ 1ml/litre water\n• Check for Zinc deficiency — apply ZnSO4 @ 25kg/ha\n• Spray in early morning or evening\n\n⚠️ This is an AI-generated answer. Please wait for expert verification.`;
  }
  if (q.includes('soil') || category === 'soil') {
    return `🤖 AI Answer: For soil health:\n\n• Get soil tested from nearest KVK (free)\n• Wheat needs loamy soil with pH 6-7.5\n• Rice grows best in clay/loamy soil with pH 5.5-6.5\n• Apply FYM @ 4-5 tonnes/acre before sowing\n\n⚠️ This is an AI-generated answer. Please wait for expert verification.`;
  }
  if (q.includes('insect') || q.includes('pest') || category === 'pest') {
    return `🤖 AI Answer: For pest control:\n\n• Spray Neem oil @ 5ml/litre (organic)\n• For severe attack: Chlorpyrifos 20EC @ 2ml/litre\n• Install yellow sticky traps\n• Scout field every 7-10 days\n\n⚠️ This is an AI-generated answer. Please wait for expert verification.`;
  }
  return `🤖 AI Answer: Thank you for your question!\n\n• Contact KVK: 1800-180-1551 (Toll Free)\n• Use our Soil Analyzer for soil health\n• Use Crop Advisor for farming advice\n\n⚠️ This is an AI-generated answer. Please wait for expert verification.`;
};
...
const askQuestion = async (req, res) => {
  try {
    ...
    const aiAnswer = await generateAIAnswer(title, category);

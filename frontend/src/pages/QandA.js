...
const QandA = () => {
  ...
  const handleAsk = async () => {
    if (!form.question) return toast.error('Please enter a question');
    setLoading(true);
    try {
      await API.post('/questions/ask', form);
      toast.success('✅ Question submitted! AI has answered!');
      setShowForm(false);
      setForm({ question: '', category: 'general' });
      fetchQuestions();
            ...
            <div className="space-y-2">
              {questions.map((q) => (
                <button key={q._id} onClick={() => setSelected(q)}
                  ...
                  style={{ borderColor: selected?._id === q._id ? '#6d28d9' : '#f0f0f0',
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-xs px-2 py-0.5 rounded-full font-semibold"
                      style={{ background: statusStyle[q.status]?.bg, color: statusStyle[q.status]?.color,
              ...
              {questions.length === 0 && (
                <div className="text-center py-12 bg-white rounded-2xl border border-dashed border-gray-200">
                  ...
                  <button onClick={() => setShowForm(true)}
                    className="mt-3 text-purple-600 text-sm font-semibold">Ask your first question →</button>
                </div>
              )}
            </div>
          </div>
            ...
            {selected ? (
              <div className="space-y-4 fade-up">
                ...
                <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
                  ...
                  <p className="text-gray-800 font-semibold text-base">{selected.question}</p>
                ...
                {selected.aiAnswer && (
                  <div className="rounded-2xl p-5 border border-blue-100" style={{ background: '#eff6ff' }}>
                    <p className="text-xs font-semibold text-blue-500 uppercase tracking-wide mb-2">🤖 AI Answer</p>
                ...
                {selected.expertAnswer && (
                  <div className="rounded-2xl p-5 border border-green-100" style={{ background: '#f0fdf4' }}>
                    <p className="text-xs font-semibold text-green-600 uppercase tracking-wide mb-2">✅ Expert Verified Answer</p>
                    ...
                    {selected.expertId && (
                      <div className="flex items-center gap-2 mt-3 pt-3 border-t border-green-100">
                        ...
                        <div>
                          <p className="text-xs font-semibold text-green-700">{selected.expertId?.name || 'Agricultural Expert'}</p>
